...
def run_dotnet_publish(folder, runtime, config, self_contained, framework):
    os.chdir(folder)
...
def main():
    ...
    configs = [
        ...
        ('win-x64', 'Release', True, 'net8.0'),
        ('win-arm64', 'Release', True, 'net8.0'),
        ('osx-x64', 'Release', True, 'net8.0'),
        ('osx-arm64', 'Release', True, 'net8.0'),
        ('linux-x64', 'Release', True, 'net8.0'),
        ('linux-arm64', 'Release', True, 'net8.0')
    ]
    ...
    for runtime, config, self_contained, framework in configs:
        ...
        run_dotnet_publish(parent_dir, runtime, config, self_contained, framework)

        # Determine the bin directory
        bin_dir = os.path.join(parent_dir, 'bin', config)
        ...
        if output_dir and os.listdir(output_dir):
            ...
            arch_folder = os.path.join(parent_dir, 'installers', runtime)
